...
static void continuous_adc_init(adc_channel_t *channel, uint8_t channel_num,
	...
	{
	.max_store_buf_size = 1024, .conv_frame_size = EXAMPLE_READ_LEN,
	};
...
void app_main(void)
	...
	while (1)
	    {
	    ret = adc_continuous_read(handle, result, EXAMPLE_READ_LEN,
		    &ret_num, 0);
	    if (ret == ESP_OK)
		{
		...
		for (int i = 0; i < ret_num; i += SOC_ADC_DIGI_RESULT_BYTES)
			...
			{
			ESP_LOGI(TAG,
				"Unit: %s, Channel: %"PRIu32", Value: %"PRIx32,
				unit, chan_num, data);
			}
		    else
			{
			ESP_LOGW(TAG, "Invalid data [%s_%"PRIu32"_%"PRIx32"]",
				unit, chan_num, data);
			}
		    }
		/**
